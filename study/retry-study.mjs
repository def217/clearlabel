...
const OUT = new URL('./results-retry.jsonl', import.meta.url);
...
const studyOne = async ({ rank, domain, cctld }) => {
  ...
  const home = await scanUrl(base + '/', false);
...
const rows = (await readFile(new URL('./retry.csv', import.meta.url), 'utf8'))
  .trim().split('\n').slice(1)
